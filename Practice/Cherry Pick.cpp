...
bool exist(int r, int c, vector<vector<long long>> &arr, int n, int m, vector<vector<long long>> &visited)
{
    if (r < 0 or c < 0 or r >= n or c >= m or visited[r][c] or arr[r][c] == -1)
        return false;
    if (r == n - 1 and c == m - 1)
        return true;
    bool f = false;
    visited[r][c] = true;
    f = f or exist(r + 1, c, arr, n, m, visited);
    f = f or exist(r - 1, c, arr, n, m, visited);
    f = f or exist(r, c + 1, arr, n, m, visited);
    f = f or exist(r, c - 1, arr, n, m, visited);
    visited[r][c] = false;
    return f;
}

void dfs(int r, int c, vector<vector<long long>> &arr, int n, int m, vector<vector<long long>> &visited, int *sum)
{
    if (r < 0 or c < 0 or r >= n or c >= m or visited[r][c] or arr[r][c] == -1)
        return;
    if (arr[r][c] == 1)
    ...
    for (int i = 0; i < 4; i++)
    {
        dfs(r + dx[i], c + dy[i], arr, n, m, visited, sum);
    }
}
int solve(vector<vector<long long>> &arr, int n, int m)
{
    vector<vector<long long>> visited(n, vector<long long>(m, 0));
    if (!exist(0, 0, arr, n, m, visited))
        return 0;
    vector<vector<long long>> vis(n, vector<long long>(m, 0));
    int sum = 0;
    dfs(0, 0, arr, n, m, vis, &sum);
    return sum;
}
void sol()
{
